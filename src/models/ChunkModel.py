...
class ChunkModel(BaseDataModel):
    ...
    async def get_poject_chunks(self, project_id: UUID, page_no: int=1, page_size: int=50):
        async with self.db_client() as session:
    ...
    async def get_total_chunks_count(self, project_id: UUID):
        total_count = 0
        async with self.db_client() as session:
            count_sql = select(func.count(DataChunk.chunk_id)).where(DataChunk.chunk_project_id == project_id) # pylint: disable=not-callable
